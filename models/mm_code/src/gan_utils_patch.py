...
class Training:
    ...
    def train_epoch(self, epoch, epochs, show_fig=True, save_images=True):
        ...
        for i, data in enumerate(pbar):
            ...
            valid_d = torch.full((batch_size, 1, 30, 30), 0.9, device=self.device) 
            #print(f"valid_d: {valid_d.shape}")
            fake_d = torch.full((batch_size, 1, 30, 30), 0.1, device=self.device)
            #print(f"fake_d: {fake_d.shape}")
    ...
    def validate_epoch(self, epoch, epochs, show_fig=True, save_images=True):
        ...
        with torch.no_grad():
            for i, data in enumerate(pbar):
                ...
                L, abs_ = L.to(self.device), abs_.to(self.device)
                batch_size = L.size(0)
                real_labels = torch.ones((batch_size, 1, 30, 30), requires_grad=False).to(self.device)
                fake_labels = torch.zeros((batch_size, 1, 30, 30), requires_grad=False).to(self.device)
...
class Visualization:
    ...
    def plot_losses(epoch, train_loss_discriminator, val_loss_discriminator, train_loss_generator, val_loss_generator, run_dir, base_dir='training_runs', show_fig=False, save_images=True):
        ...
        axs[2].plot(epoch_range, (0.01 * train_loss_generator), c="g", label="Generator Train Loss")
