    ...
    def train_epoch(self, epoch, epochs, show_fig=True, save_images=True):
        ...
        for i, data in enumerate(pbar):
            ...
            if show_fig or save_images:
                self.visualize_images(L, generated_abs, abs_, epoch, i, show_fig, save_images)
                show_fig = False
                save_images = False
...
class GANDriver:
    def __init__(self, generator, discriminator, train_dl, val_dl, optimizer_G, optimizer_D, adversarial_loss, content_loss, lambda_l1, device, epochs, run_dir, base_dir='training_runs'):
        """
        ...
        self.trainer = Training(self.generator, self.discriminator, self.optimizer_G, self.optimizer_D,
                                self.adversarial_loss, self.content_loss, self.lambda_l1, train_dl, self.device, run_dir, base_dir)
        ...
        self.run_dir = run_dir  # Store the run directory for saving images and weights
    ...
    def run(self, show_fig=True, save_images=True):
        ...
        for epoch in range(self.epochs):
            ...
            train_d_loss, train_g_loss = self.trainer.train_epoch(epoch, self.epochs, show_fig=show_fig, save_images=save_images)
            ...
            self.visualizer.plot_losses(epoch, self.train_loss_discriminator, self.val_loss_discriminator,
                                        self.train_loss_generator, self.val_loss_generator, run_dir=self.run_dir, base_dir=self.base_dir, show_fig=show_fig, save_images=save_images)
